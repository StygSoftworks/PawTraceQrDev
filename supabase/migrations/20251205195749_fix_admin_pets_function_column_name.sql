/*
  # Fix Admin Pets Function Column Reference

  ## Overview
  Fixes the get_all_pets_admin() function to use the correct column name.
  The profiles table has `display_name`, not `name`.

  ## Changes Made
  - Updated the function to select `profiles.display_name` instead of `profiles.name`
  - This resolves the 400 error when calling the function via RPC

  ## Security
  - Maintains all existing security checks (role verification)
